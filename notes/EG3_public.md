...
In-paper anchor: `paper/MINIMAL_MODEL_PROGRAM_PREPRINT.md` (`MMP_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving existence and persistence of the declared minimal-model and Mori-fiber endpoint packages across admissible birational reductions through a multi-lane birational super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`MMP_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_MMP`.
...
If near-failure extraction and no-Zeno spacing hold, then `MMP_G3` prevents hidden drift before rigidity is applied.
