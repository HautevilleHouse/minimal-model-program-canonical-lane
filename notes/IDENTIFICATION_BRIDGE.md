...
In-paper anchor: `paper/MINIMAL_MODEL_PROGRAM_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving existence and persistence of the declared minimal-model and Mori-fiber endpoint packages across admissible birational reductions through a multi-lane birational super-architecture`.
...
- endpoint lock: `semiample_transfer`.
...
2. the endpoint-transfer lock `semiample_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_MMP` remains positive.
...
Payload: verify `semiample_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_MMP` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `semiample_transfer`
